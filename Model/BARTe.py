...
class BARTeModel(pl.LightningModule):
	...
	def train_model(self):
		...
		path_save=f'/data/rali6/Tmp/piedboef/Models/FKPG/{self.argdict["dataset"]}_BARTe_{self.argdict["num_epochs"]}Epochs_random_seed_{self.argdict["random_seed"]}.pt'
	...
	def compare_correct_kp(self, dataset):
		...
		with torch.no_grad():
			...
			for dat in dataset:
				...
				# src_text = " ".join(dat[self.field_input].split(' ')[:ll])
				# src_text = src_text
				# src = self.tokenizer(dat[self.field_input], padding=True, truncation=True)
				# target = self.tokenizer(batch['full_labels'], padding=True, truncation=True)
				input_ids = self.tokenizer(dat[self.field_input], padding=True, truncation=True, return_tensors='pt',
										   max_length=self.argdict['max_seq_length']).to(self.device)
				# input_ids = self.tokenizer.encode(dat[self.field_input], return_tensors='pt', truncation=True, max_length=self.argdict['max_seq_length']).to(self.device)
				# print(input_ids)
				# print(self.tokenizer.batch_decode((input_ids)))
